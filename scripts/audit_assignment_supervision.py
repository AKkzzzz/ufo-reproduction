...
def main():
    ...
    for chunk_index, (inputs, _) in enumerate(chunks):
        ...
        per_time = anchors.shape[1] // t
        anchors_t = anchors.reshape(b, t, per_time, 3)
        valid_t = anchor_valid.reshape(b, t, per_time)
        labels = []
        predicted_labels = []
        for time_index in range(t):
            lidar_prob, _ = construct_assignment_targets(
                torch.zeros_like(anchors_t[:, time_index]),
                inputs["context_instances_corner"][:, time_index],
                inputs["context_instances_id"][:, time_index],
                "lidar_anchor", anchors_t[:, time_index], valid_t[:, time_index],
            )
            predicted_prob, _ = construct_assignment_targets(
                anchors_t[:, time_index] + 1000,
                inputs["context_instances_corner"][:, time_index],
                inputs["context_instances_id"][:, time_index],
                "predicted_mean",
            )
        ...
        rows.append({
            ...
            "valid_anchor_count": int(valid_t.sum()),
            "dynamic_anchor_count": int(dynamic.sum()),
            "dynamic_ratio_supervised": float(dynamic.sum() / valid_t.sum().clamp_min(1)),
            "example": {
                "time": time_index, "camera": camera, "patch_y": patch_y,
                "patch_x": patch_x, "token_local": token_local,
                "anchor_xyz": anchors_t[0, time_index, token_local].tolist(),
    ...
    result = {
        ...
        "paper_status": {
            ...
            "lidar_anchor_label_construction": "REPRODUCTION_DECISION",
            "anchor_hidden_width_or_sampling_policy": "UNKNOWN",
        },
    }
